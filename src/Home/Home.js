  ...
  update = () =>{
    ...
    for(var i=0;i<Object.keys(this.state.monuments).length;i++){
      if (distance(this.state.marker[0],this.state.marker[1],this.state.monuments[i].Latitude,this.state.monuments[i].Longitude)<=vardis && this.state.near==false){
        ...
        idee_monument.id = this.state.monuments[i].id_monu;
        store.addNotification({
  title: "Vous êtes près de " +this.state.monuments[i].nom_monu  ,
  message: <Link to="/Descriptif"><Button className="btn_popup" variant="info" size="sm" block>
               Plus d'informations
           </Button></Link>,
  type: "info",
  insert: "top",
